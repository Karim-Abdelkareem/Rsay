...
export const HoverEffectServices = ({ items, className }) => {
      ...
      {items.map((item, idx) => (
        ...
        >
          <AnimatePresence>
            {hoveredIndex === idx && (
              <motion.span
                className="absolute inset-0 h-full w-full bg-brand/20 dark:bg-slate-800/[0.8] block  rounded-3xl"
...
export const CardTitle = ({ className, children }) => {
  return (
    <h4
      className={cn(
        "text-zinc-950 font-bold md:text-lg tracking-wide mt-4",
        className
      )}
    >
      {children}
